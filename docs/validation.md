...
## 324. Layer 4 moved into the agent account's own session, so verifying a build no longer costs somebody their desktop (2026-09-23)

§323 row 863 left one thing owed, and the owner's 「你自己看着处理」 made it this
round: the console gate's refusal was a stopgap — it *asked* before clearing the
slate, where the real fix is a gate that never needs the slate. `scripts/gui-verify.sh`
has to close every AgentSpace copy this uid owns and then drive a window of its own
on the human's screen for a minute or two, because all 13 of its checks read an
accessibility tree that only exists where windows are being presented. The agent
account's session presents its own windows, is invisible from the human's screen,
and already has the Accessibility grant the checks need — so that is where they can
live.

The instrument question came first, and it was measured rather than assumed. Inside
that session, `osascript` driving System Events — the technique every one of the 13
console checks is written in — **does not work**: an AppleEvent with a 20-second
timeout answers `-1712 AppleEvent timed out`. What does work is the Accessibility
API itself, called directly by a process the worker spawned: `AXIsProcessTrusted()`
is `true` there, TextEdit's tree (2 windows, 46 nodes, its own `AXIdentifier`s) reads
back normally, and `screencapture` works too. So the port could not be a copy of
`gui-verify.sh` with a different session in it; it is the same 13 claims driven
through `AXUIElement` instead of through AppleScript, and the two reports can be read
side by side because the check names and the order are identical.

`tests/SessionUI` (product `agentspace-gui-check`) is that program, and
`scripts/session-gui-verify.sh` is the way to run it: it builds both binaries,
resolves the account, refuses with a sentence if the session cannot answer, hands the
checker to that session through the worker's shipped `exec`, and reads the checker's
exit code back through a channel that flattens it. Everything it needs already ships
in the installed worker — the installed pair on this machine is app `0.1.31` /
worker `0.1.27`, so the gate was run, not promised.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 864 | The agent account's session can host the app under test and vend a readable accessibility tree, while System Events cannot be driven there at all | pass (measured in that session, 2026-09-23) | `agentspace exec AgentUse /tmp/probe.sh 21213` answered `whoami=agentuse uid=503`, `trusted=true`, `windows=2`, `nodes=46`, `identified=6` (including `First Text View` on TextEdit pid 21213), and `screencapture: ok 1875726`; the same script's `osascript -e 'with timeout of 20 seconds … tell application "System Events" …'` answered `71:101: execution error: "System Events"遇到一个错误：AppleEvent已超时。 (-1712)`. Both halves come from one run of one script, so the difference is not the environment between runs — it is the channel |
| 865 | A process spawned by the worker's own `exec` inherits the worker's Accessibility grant, which is what makes this gate possible without asking the owner for anything | pass (measured) | `/tmp/axprobe` (a 40-line `xcrun swiftc` program) printed `trusted=true pid=21213` when run through `exec`, and read TextEdit's window titles and identifiers; `agentspace status AgentUse --json` reports `accessibility: true` for the worker itself. The grant is not re-asked per binary: TCC attributes the spawn to the worker. The same program run from a login shell never gets that far — it is refused by the console guard below (§324 row 869) |
| 866 | Three reading traps were measured before writing the checks, each of which would have failed silently: menu key equivalents arrive **uppercase** (`N` for 「新建 Agent…」), the Settings window is the one with **five** toolbar buttons while the dashboard has four of its own, and a closed SwiftUI picker's menu items are **not** in the tree | pass (measured) | One probe run against the launched app dumped the whole menu bar's key equivalents — `A menubar cmdchars=[⎋:强制退出…,Q:锁屏,…,N:新建 Agent…,W:关闭,…]` — in which the Settings item is `,` and New Agent is `N`. The console gate matched `n` only because AppleScript's `is` folds case, so the port compares case-insensitively (`AXUI.menuItem`, `tests/SessionUI/AXUI.swift`): a case-sensitive port would have failed to open the wizard while looking exactly like a build defect. The window dump read `A win[0] title=[账户] toolbars=1 toolbarButtons=5` (账户/权限/性能/高级/Update) beside `A win[1] title=[AgentSpace] toolbars=1 toolbarButtons=4` (隐藏边栏/应用/打开桌面/打开应用) — which is why the Settings window is found by shape, not by `window 1` (§299 rows 639–640). And `A displayQualityPicker: role=AXPopUpButton value=[原生 Retina]` with `picker children: ` empty — the menu is not readable while closed, so the quality check falls back to the six shipped spellings (§323 row 860's finding, now measured from the other side) |
| 867 | All 13 of layer 4's checks pass inside the agent session against the gated bundle | pass (measured, three runs) | `scripts/session-gui-verify.sh` on `dist/AgentSpace.app` (0.1.31, `CFBundleVersion` 413, the stapled release artifact): `13 passed, 0 failed`, in 34.8 s (and 41.9 s twice for the manual runs). The report names them: one window, `构建 0.1.31 (413)` in `appBuildVersion`, the scratch root in `registryRootOverrideNotice`, `refresh slider min=2.0` / `max=10.0`, `display quality control`, the five Update-pane facts (`installButton=false`), the wizard, and `dead link alert` = `SPACE_NOT_FOUND`. Nothing in that run was on the human's screen |
| 868 | A run leaves the owner's own AgentSpace untouched — and the tool cannot touch it, which is a stronger statement than "it did not this time" | pass (measured) | Before and after a full gate run, on this machine: `pgrep -U "$(id -u)" -f 'AgentSpace.app/Contents/MacOS/AgentSpace'` answered `15493` both times with an identical `lstart` (`Wed Sep 23 14:43:56 2026`), so the process was neither signalled nor relaunched. The mechanism is structural: the checker runs as `agentuse`, and `pkill -x AgentSpace` from inside that session answers `pkill: signalling pid 15493: Operation not permitted`. The console gate needed a consent guard to stop doing this (§323 row 863); this one cannot do it at all |
| 869 | The checker refuses rather than disturbing, if it is ever started in the wrong session — and it refuses *before* launching anything | pass (measured) | `.build/debug/agentspace-gui-check --bundle dist/AgentSpace.app` from a login shell in the console session printed `this process is running in the session that is on the physical screen … Nothing has been launched.` and exited **3**. The premise is enforced with the console gate's own probe (`CGSessionCopyCurrentDictionary`, the two-S `kCGSSessionOnConsoleKey`, CFBoolean-or-CFNumber values), and an unreadable state is refused too (`tests/SessionUI/main.swift`, "Session preconditions") |
| 870 | "Could not verify" is now distinguishable from "verified and failed", and the release gate falls back instead of stopping | pass (measured end to end) | `scripts/gui-verify.sh` exits **3** for all four of its environment/consent refusals (locked screen, not on the console, unreadable state, a running copy it will not close) and 1 only when checks ran and failed — this contradicts §297's parenthetical "the refusal (`exit 1` …)", which was true when 1 was also the pass-fail code. `scripts/check-all.sh`'s `run_layer` catches 3 for that script only, runs `scripts/session-gui-verify.sh`, and prints `    gui-verify verified nothing (exit 3, above) — running its session-side twin` so the transcript always names which instrument produced the layer-4 verdict. Measured with the owner's copy running: layers 1–3 passed, the console gate refused and listed that copy (`15493 /Applications/AgentSpace.app/Contents/MacOS/AgentSpace`), the fallback line printed, the session gate reported `13 passed, 0 failed`, and the run ended `check-all: all 4 layers passed` / exit 0 |
| 871 | The checker's exit code survives a channel that flattens it, so a refusal cannot be read as a pass | pass (measured) | The CLI's `exec` maps any non-zero child exit to 1 (`native/AgentSpaceCLI/Sources/AgentSpaceCLI/main.swift`, the exec verb), so the wrapper appends `; printf "agentspace-gui-check exit: %s\n" "$?"` to the command the worker's shell runs — one argv element, `$?` single-quoted so the *inner* shell expands it — and reads the code back from there. Measured through the channel: `0` for a passing run, `3` for the console refusal, and both are what the wrapper propagates |
| 872 | The wrapper refuses, with exit 3 and a sentence naming the fix, when the session gate cannot answer at all | pass for the cases this machine has; the rest is code-and-refusal-shape only | Measured here: `--account NoSuchAgent` → `could not read the status of 'NoSuchAgent'` + `SESSION_NOT_READY` from the worker, exit 3; `--bundle /tmp/nope.app` → `no app bundle at /tmp/nope.app`, exit 3. Implemented but **not exercised** on this machine, because it has exactly one attached account and its worker is up: the several-accounts ambiguity refusal, the worker-not-answering refusal, and the `onConsole` refusal. Those three are the ones a second machine would test |
| 873 | The wizard branch this gate reaches is the *empty account state*, and that is correct for the session rather than a weakness of the port | pass (measured), with the consequence stated | Transcript: `wizard: empty account state` and `ok   wizard explains how to add an account`. Account discovery excludes the **current** user and already-attached usernames, so a wizard opened as `agentuse` — itself attached, with the human excluded as an admin — has nothing to offer, while the console gate run as the human reaches `step 2` (§301 row 654). Both are real states and the check accepts both, but the *review card* path is therefore still only covered by the console gate |
| 874 | What this retires: layer 4 being "deferred rather than refused" because the console was locked or somebody's window was open | pass (measured) | The two recorded deferrals (§310 row 779, §313 row 792 — `gui-verify` skipped at release time to leave the owner's window and a soak's subject alone) are exactly the states `run_layer` now answers from the session instead. A locked screen or a busy machine no longer costs the layer: row 867's gate ran on this machine *with the owner's app running*, which is the configuration that previously stopped `check-all.sh` at its fourth layer |
| 875 | Not measured, and named rather than implied | open | (a) The session gate needs an attached account with a live worker; with neither, `run_layer` falls back to a refusal and the release stops exactly as before — it is an *additional* instrument, not a replacement. (b) The checker drives controls through `AXPress` only; it does not exercise the worker's input path (`agentspace click`/`key`/`scroll`), so it is not evidence about input injection. (c) It never asserts the two dismissal behaviours the console gate depends on (Escape, ⌘W) — it closes windows through their own close buttons and relaunches between phases instead. (d) The console gate remains the only one that verifies an *installed* copy end to end (its §302 `TMPROOT` note); this one always tests the bundle it is pointed at. (e) The three refusals in row 872 are unexercised here. (f) The picker's three option titles are asserted from the six shipped spellings, not from the control's own items (row 866). (g) A build whose Settings window or wizard never appears stops the run (exit 1, after closing the subject and removing the scratch roots) rather than scoring the remaining checks — so in that case the report is a named missing surface, not `N passed, M failed` |

## 325. Fusion could only mirror what was already running — the app picker, and the three plist/name traps measuring it turned up (2026-09-23)

The gap this round closes is the one the product's own headline sentence had
skipped. 「可以选择 Safari / Terminal / VSCode 单独融合」 was reachable only if
that app happened to be **already running** in the agent's session: the Fusion
entry point (`FusionManager.openApps`) mirrors the windows `window.list` reports,
and nothing in the product could answer "what is installed here, and would you
like one of those started". An agent could launch an app by name (`launch`), but
a person looking at the app had no list and no search — the picker did not exist.

`apps.available` is that missing question on the wire (additive at protocol
version 1: `apps` keeps answering exactly what it always has, and an older worker
answers the ordinary `unknown method` error), `ApplicationCatalog` in Core holds
every rule that decides what the list contains, and `FusionAppPickerView` is its
surface — a search field, icons, and one button per row that either starts the
app in that session or, when it is already up, shows the windows it has.

The numbers below come from a worker **built from this tree** running in the
agent account's own session (`Probe`, pid under `/tmp/asv`, the recipe §322 row
843 established), because the installed worker is `0.1.27` and does not know the
method — which is itself row 882.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 876 | The session's installed applications can be listed, deduplicated and capped, and the answer is a real one rather than an empty scaffold | pass (measured in the agent session) | `agentspace --root /tmp/asv apps Probe --available` against the scratch worker: `count=224 total=224 truncated=False`, names/ids/paths/versions per row (`Activity Monitor com.apple.ActivityMonitor 10.14 /System/Applications/Utilities/Activity Monitor.app`). Cap and truncation are visible in the other direction: `--all` answers `count=250 total=372 truncated=True` |
| 877 | The default list is the apps a person can open **and look at**: menu-bar agents and background-only services are kept out, which removed 148 of 372 entries | pass (measured, and it took two attempts) | Before the rule: 372 entries, `AddPrinter`, `AddressBookUrlForwarder`, `AccessibilityUIServer` among them — no window to fuse, in a list meant to be scanned by eye. First fix (`as? Bool`) removed only 16: these bundles spell the flag `LSUIElement = YES` (an `NSTaggedPointerString`) and `LSBackgroundOnly = 1`. `ApplicationCatalog.readsAsTrue` accepts Bool, number and the strings `1/true/yes` (10 cases pinned in `ApplicationCatalogTests`), and the same daemons are now absent from the default reply: `守护进程是否还在: []` |
| 878 | Every row has a readable name, with no `.app` on it | pass (measured twice, both attempts a real defect) | First measurement: a row with **no name at all** — `/Applications/三局通.app` carries an empty `CFBundleDisplayName`, and an `??` chain does not catch the empty *string*. Second: `FileManager.displayName(atPath:)` answers 「活动监视器.app」 — localized, but with the extension Finder does not draw. `ApplicationCatalog.displayName` prefers the system's answer, strips a trailing `.app`, and treats whitespace as empty. After the fix, across 224 rows: `空名=0 带.app后缀=0` |
| 879 | Icons travel with the record, at a size that makes the reply readable rather than enormous | pass (measured byte counts) | `apps --available --icons --json`: **224/224** rows carry one, PNG header `89504e470d0a1a0a`, **32×32**, 1,300–2,566 bytes each (mean 2,016), whole reply **644,252 bytes**. `NSImage.size` + `tiffRepresentation` — the obvious way to shrink an icon — returns the 512-point original instead (~100 KB per app), which is why the draw goes through an explicit `NSBitmapImageRep`. Icons are rendered by the worker because a bundle in the agent account's own home is not readable by the controller user |
| 880 | A row the picker offers is an app `launch` can find: one directory list, used by both | pass (measured + test) | `AppControl.resolve`'s hand-written six-root array is gone; it calls `ApplicationCatalog.searchRoots(home:)`, and `AppCatalog` scans the same function. The reply carries `roots` so a caller can see where it looked. `testHomeApplicationsComeFromTheGivenHome` pins the list and that no root repeats |
| 881 | The picker works end to end in the agent account's own session, against the tree's worker — and search filters it | pass (measured on screen, in that session) | The app bundle built from this tree (`scripts/bundle-app.sh debug /tmp/asgui`, never `dist/`) launched there with `AGENTSPACE_ROOT=/tmp/asv`, read through the accessibility tree by a probe: `picker button enabled=true`, `press picker -> 0`, `search field present; visible rows=80` with names and paths (`Activity Monitor`, `/System/Applications/Utilities/Activity Monitor.app`), then after typing `safari`: `visible rows=5 names=["?", "Safari", "/Applications/Safari.app", "已在运行", "?"]`. `80`/`5` are the rows the sheet's lazy stack materialized, not the list's length — the search itself is anchored to the presence of the row for Safari, and 「已在运行」 is the localized 「Running」 badge, which is also row 883's `pid` field doing its job |
| 882 | The addition is compatible: no method renamed, `protocolVersion` still 1, and an older worker says so instead of answering something wrong | pass (measured against the installed 0.1.27 worker) | `agentspace apps AgentUse --available` on the installed worker: `agentspace: METHOD_NOT_FOUND: unknown method 'apps.available'` with the doctor hint — a named refusal, not an empty list. `agentspace apps AgentUse` (the old verb) still answers the running list unchanged. `Method.appsAvailable` is a new constant; nothing was renamed or given a second meaning |
| 883 | The reply distinguishes what is already running, so a picker offers windows instead of a second copy | pass (measured) | The Safari row in row 881's transcript carried the 「已在运行」 badge, and `AppCatalog` sets `pid` by matching the running list on bundle id **or** path (the path arm covers an app this session launched from a directory `resolve` would not search). `ApplicationCatalog` is where the JSON is shaped, so the CLI prints `←running` on the same field (`Safari com.apple.Safari 27.0 /Applications/Safari.app ←running`) |
| 884 | The GUI's new surface is covered where it can be, and its strings exist in both languages | pass (measured) | 16 new tests in `tests/Unit/ApplicationCatalogTests.swift` pin the rules (launchability, dedup by case-insensitive bundle id, first-root-wins, ordering, the three search fields, empty/whitespace queries, the plist flag spellings, the name precedence, the optionals' meaning); the full suite is **616 tests, 0 failures**, which includes `LocalizationTests`' key-parity and prose scans over the eleven new keys in both `.lproj` tables |
| 885 | Not measured, and named rather than implied | open | (a) The **launch** action itself was not exercised live: the row the probe reached (Safari) was already running, so the measured path is the 「显示窗口」 one, and `FusionManager.fuse`'s launch arm is covered by code and by `SpaceService`'s existing `launch` (which §322 measured) rather than by this round's screen evidence. (b) 收藏 (favorites) is not implemented; only 最近使用, which is `UserDefaults` per Space (five entries). (c) The agent session's own `~/Applications` is empty on this Mac, so the root that motivated icons-travel-with-the-record was not exercised with a real app. (d) The picker has **no gate check**: adding a 14th check to layer 4 would have to go into *both* instruments to keep their verdicts comparable, the console one is AppleScript that needs a console session to develop, and shipping an unrun check into a release gate is the defect class §307 and §318 are about. Owed as its own round, and named in `status.md`. (e) Multi-display pointer input (input is still bounded to the main display) is unchanged by this round |
| 886 | A screenshot taken of a sheet that was not frontmost looked exactly like a layout defect, and the geometry said otherwise | pass as a caution, no code change | The picker's rows were read off `/tmp/picker-list.png` as `Reader`, `tor`, `HP` — text cut at the sheet's left edge. The accessibility geometry for the same moment: `LIST x=670 w=580`, the name at `x=704 w=122` ending at 826, which is exactly where the rendered text ended. The cut was at **x≈815 — the right edge of the agent session's own TextEdit window**, which was in front of the AgentSpace window because the app had been started by `exec` rather than activated. Not a layout bug: after `NSRunningApplication.activate` the same sheet renders 「选择要融合的应用」, 「搜索应用」, four rows with their icons, names, paths and 「融合」/「显示窗口」 buttons. The lesson is about the instrument, not the view: **a screenshot of a window that may not be frontmost is not evidence about that window's layout** — check what is in front before believing the pixels, and prefer geometry (which is what found it). Two comments written on the false diagnosis were rewritten; the sizing they introduced (`.frame(width:height:)`) was kept, for the honest reason that a picker should not size itself to its longest path |

## 326. 0.1.33's release chain: a second release inside the same hour, and the gate that no longer needs anybody's desktop (2026-09-23)

0.1.32 was published with no product change (§324). Two hours later the app picker
was built (§325), and this section is the chain that put *that* in front of a user:
bump, `release.sh`, `notarize.sh`, `check-all.sh`, publish, read back.

What is new about this chain is what layer 4 did. The owner's app was running for
the whole of it — 13/13 came from the **agent session**, not from their screen —
so this is the first release in this repository's history whose gate ran to green
without a published artifact being verified in front of somebody. §310 row 779 and
§313 row 792 are the two releases that had to *skip* that layer for exactly this
reason; row 870's fallback is what retired the trade.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 887 | The release chain ran end to end on the picker's tree, and each step is read back rather than assumed | pass (measured) | `4e7b1c1` bumped to 0.1.33 (four version strings); `scripts/release.sh` → `dist/AgentSpace-0.1.33.dmg`, Developer ID `Guofeng Liu (U8U443D7ZL)`, every nested binary verifying; `scripts/notarize.sh` → **Accepted** for both submissions (`b758ac4d-bb32-43a4-843d-cc474f6b7e3a` app, `ab7c8e69-7e44-4fec-b0cf-8c9ad191642f` DMG, read back from Apple's list, not from the script's own log), stapled, `Gatekeeper accepts the app` |
| 888 | Layer 4 verified the artifact without closing or driving anything on the owner's session — the first release where that is true | pass (measured on the gated bytes) | `scripts/check-all.sh`: `==> scripts/gui-verify.sh` refused with `an AgentSpace copy is running in this session`, `gui-verify verified nothing (exit 3, above) — running its session-side twin`, `agentspace-gui-check (AgentSpace): 13 passed, 0 failed`, `session-gui-verify: 13 checks passed in AgentUse's session (nothing on this screen)`, and `check-all: all 4 layers passed` / exit 0. The other three layers passed on the same run |
| 889 | The published asset is the byte-for-byte artifact the gate ran on, and it is what the product's own update channel offers | pass (measured against GitHub) | Local `sha256:a7f870228dfb49503205f0536f1fb207716873544ca153eb1b1031175a16edbc` over `5709522` bytes; the release's asset digest and size are **identical**, and `GET /repos/misswell/AgentSpace/releases/latest` — the endpoint `SoftwareUpdater.latestReleaseURL` reads — answered `v0.1.33` on three consecutive samples **and** carried that digest in its asset list. No `make_latest` write was needed this time, unlike §318 row 820 |
| 890 | The tag sits on the commit whose tree the bundle was built from | pass (measured) | `git ls-remote origin refs/tags/v0.1.33` → `4e7b1c1`, which is the bump commit; the bundle's `CFBundleVersion` is **422**, equal to `git rev-list --count HEAD` at that commit, and its `CFBundleShortVersionString` is 0.1.33 |
| 891 | The release notes say the thing a person has to do, and say it first | pass (by construction, and it is a *new* shape of the two-press rule) | 0.1.33's notes open with 「这一版要按两下，而第二下才是让功能可用的那一下」: 「检查更新」 brings the picker's surface, but the question it asks (`apps.available`) is answered by the **worker**, and the installed worker is 0.1.27 — so without 「重新安装助手…」 the picker reports `METHOD_NOT_FOUND: unknown method 'apps.available'` (measured in §325 row 882). Earlier two-press releases put new *behaviour* in the worker; this one puts a new *question* there, which is why the sentence is the first line rather than a footnote |
| 892 | Two screenshots of the picker are attached to the release, taken on the agent account's display | pass (measured; no video) | `fusion-picker-search.png` (39,409 bytes: the sheet after typing `safari` — one row, 「已在运行」, 「显示窗口」) and `fusion-picker-list.png` (64,294 bytes: five rows with icons, names, paths and 「融合」 buttons). Both were captured with `screencapture -R` inside the agent session, with the app activated first — §325 row 886 is the reason that step is written down. **No video was recorded**; there is none in this repository for any release |

## 327. The desktop viewer can strand its final hover; input transport is measured before adding another channel (2026-09-23)

The desktop viewer uses `PointerTravelCoalescer` at 30 Hz. A hover offered
inside the interval after the previous RPC completed stayed pending until a
later mouse event called `pump`. When a hand stopped there, that event never
arrived. The viewer now schedules one wakeup for exactly the remaining interval,
and cancels it when its surface closes. `pendingDelay` is tested with a final
rate-limited position and no later offer. No wire method or safety guard changed.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 893 | The last rate-limited hover is sent without another mouse event | pass (code + test) | `DesktopViewerInput.schedulePendingTravel()` arms a cancellable wakeup when `PointerTravelCoalescer.pendingDelay()` reports a waiting position; the focused seven-test suite includes `testFinalRateLimitedPositionReportsWhenItCanBePumped` and passed 7/7. The full Swift suite before this change passed 624/624; the release gate will run it again on the final tree. |
| 894 | Input RPC transport is already a Unix socket and does not explain a 50 ms movement delay on this host | pass (measured for RPC only) | A scratch worker built from this tree ran as uid 503 inside AgentUse's own Aqua session, with a separate `/tmp/asv-bench-20260923` registry and token; installed worker and owner session were untouched. 100 same-position `move` requests, each opening a new Unix socket as `WorkerClient` does: p50 **0.74 ms**, p95 **3.98 ms**, max **14.86 ms**. 20 `click` requests: p50 **1.74 ms**, p95 **2.83 ms**, max **5.83 ms**. These numbers end at the worker's reply, not at a rendered pixel, and therefore do **not** prove the product's <50/<100 ms visual targets. The scratch worker was stopped and its exact root removed. |
| 895 | The installed worker still has the original no-frontmost bug even though the published source fixed it | pass (measured compatibility gap) | `/Applications/AgentSpace.app/Contents/Helpers/agentspace move AgentUse 400 400` returned `NO_INPUT_TARGET: no app is frontmost ... events would be delivered to nothing` from the installed worker; the scratch worker from this tree returned success for all 100 moves. The user-facing remedy remains 「重新安装助手…」 after updating the app: an in-app update intentionally replaces only `/Applications/AgentSpace.app`. |
| 896 | The 0.1.34 update asset exists and matches the notarized local DMG | pass (read back; GitHub CLI list was stale) | `gh release view v0.1.34 --json assets` returned `[]`, but `gh release upload` was refused with `ReleaseAsset.name already exists`, and `GET /repos/misswell/AgentSpace/releases/latest` showed `AgentSpace-0.1.34.dmg`, size **5,743,323**, digest `sha256:1bb2789a75b9f0811387ac675c78d85eaecb3a8801286144ef5c50ac9953a9d8` — equal to `shasum -a 256` of the local notarized and stapled DMG. The REST endpoint is the updater's authority; the GraphQL-facing list was not. |
| 897 | 0.1.35 is the tested, notarized artifact offered by the update channel | pass (read back) | `3846fef` is the code + validation commit; `v0.1.35^{}` resolves to it on GitHub, and `CFBundleVersion` **426** equals that commit's count. `scripts/release.sh` built `dist/AgentSpace-0.1.35.dmg`; `scripts/notarize.sh` completed App submission `6a65b3fa-2410-49e5-8676-2131637c3def` and DMG submission `ef987ae0-6c87-46d9-a920-2f8f873d57c4`, both independently read back as **Accepted**, with staples valid and Gatekeeper accepting the App. `scripts/check-all.sh` passed all four layers: Swift **625/625**, MCP smoke, agent-session GUI **13/13** while the owner's App stayed open. Published Release `v0.1.35`; `GET /repos/misswell/AgentSpace/releases/latest` returned the tag and `AgentSpace-0.1.35.dmg`, **5,745,316** bytes, `sha256:08b04544ec29f343925bbb62c5fdde603907689b5d85bfdd6606cd2bdeab4457` — identical to the gated local DMG. |


## 328. Live mouse drag phases replace release-time replay (2026-09-23)

A person's drag over the desktop viewer or a Fusion proxy must begin while the
local button is still down. The prior tracker emitted one `drag` only on release,
and the worker replayed it with fixed sleeps. The new `pointerDown`,
`pointerDrag`, and `pointerUp` actions preserve the old atomic `drag` wire method
for existing clients. The desktop viewer serializes phases on one queue; Fusion
keeps its per-window queue and maps every phase against the window frame sampled
at mouse-down, because the target may move during the gesture. Session/desktop
safety checks still run for every phase.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 898 | Old drag latency is dominated by release-time replay and fixed waits | pass (measured) | Before the change, three atomic drag RPCs to safe desktop points in an isolated worker running inside AgentUse's own Aqua session took **703.5, 689.5, 663.0 ms** (p50 **689.5 ms**), and none was sent until local mouse-up. The worker's `drag` path includes 30 ms + 80 ms + 24 × 16 ms + 80 ms sleeps. |
| 899 | The new drag begins and moves before the local button is released | pass (live in agent session) | `RemotePointerGestureTests.testDragStartsBeforeTheButtonIsReleased` first failed against the old tracker and passes with live phases. With a scratch worker in AgentUse's own session, a System Settings window moved from `(658,30)` to `(733,67)` **before** `pointerUp`; the frame was unchanged after up. Six phase RPCs took **7.9, 4.8, 2.1, 4.1, 4.1, 5.3 ms** (p50 **4.44 ms**). This is request-to-reply and window-frame evidence, not a measurement of displayed pixel latency. |
| 900 | Moving a Fusion target window does not change the coordinate frame halfway through a gesture | pass (unit) | `RemoteWindowInputTests.testLiveDragKeepsTheMouseDownFrameAsWindowMoves` passes a later window at `(260,140)` with a mouse-down frame at `(200,100)` and verifies the drag still resolves to `(300,150)` → `(400,200)`. The worker keeps that frame by window identity until `pointerUp`. |
| 901 | Fusion drag is proven end-to-end on this host | open | Direct `window.input` calls against System Settings, Firefox and TextEdit in the scratch session were refused by the existing strict AX-window match (`no accessibility window matches ...`), before any new drag event was posted. This round does not weaken window targeting or claim a Fusion live result from the unit test. |
...
## 330. Desktop input moves to a persistent binary channel, the pointer is captured, and the cursor leaves the video (2026-09-23)

The desktop viewer's pointer went through the JSON RPC an agent uses — one
socket per event, encoded request, synchronous reply. Measured on this machine
that was p95 3.98 ms per call (§327 row 894), which is invisible at the default
5 FPS and obvious at 60. This round adds `input.sock`: one persistent
connection per account, fixed binary records, no reply in the hot path, and a
cursor channel that publishes the session pointer and its shape separately from
the frames. The viewer now captures the pointer on entry (`MouseCapturePolicy`,
default Auto), hides the local cursor with a scoped cursor rect rather than the
process-wide `NSCursor.hide()`, sends every press phase immediately, and draws
the agent's own cursor locally instead of waiting for a frame.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 907 | The fast channel's framing round-trips every field, refuses a wrong magic, a future version and an oversized payload, and enforces one direction per packet kind | pass (unit, 19 tests) | `tests/Unit/InputFastProtocolTests.swift`, 19/19 in `swift test --filter InputFastProtocolTests`. The oversized-payload case matters because the size field is attacker-controlled: `InputPacketHeader(decoding:)` refuses anything above `maxPayloadBytes` **before** the payload is read |
| 908 | The input socket's own handshake is measured against a worker on this machine, and the write cost is well under one display refresh | pass (measured, live worker) | Scratch worker from this tree, uid 501, throwaway root `/tmp/as-input-probe`, owner's session untouched. One connection: handshake accepted with protocol 1 and `[absolutePointer,windowPointer,scroll,keyboard,leaseControl,cursorPosition,pressPhases,applyTimestamps]`; **500** pointer packets written down that one socket: **p50 0.221 ms, p95 0.255 ms, p99 0.276 ms, max 0.329 ms**. The same worker answered 50 `input` RPCs at p50 0.461 ms / p95 0.597 ms — the comparison the round exists for. This is the *write*, not a rendered pixel |
| 909 | A console session refuses every fast-channel packet with the same code the RPC path answers | pass (measured, live worker + gate) | Live: 500 moves and a press/up/scroll/key/type run against the scratch worker, whose session verdict is `isConsole`, answered `refusedSession` with the RPC path's own sentence. Recorded as a gate rather than only as a transcript: `tests/Safety/SafetyTests.swift` `testFastInputChannelEnforcesTheSameSessionRuleAsTheRPC` starts a real worker, opens a real `input.sock` and asserts the refusal — 24/24 safety tests green |
| 910 | A flood of refusals is throttled, and the fix was needed because the naive behaviour deadlocked the worker | pass (measured, then fixed) | First live run: 500 refusals produced 500 ack packets and the connection died at packet **196** with `sendTimeout` — the worker filled its own socket buffer. `InputAckPolicy` now samples a *repeating* refusal (one per 32 packets), always sends the first one and always sends a *changed* one, so a transition is never lost. Re-measured on the same worker: 500 packets → **16** acks, no write failure. Unit-pinned by `testAFloodOfRefusalsIsSampledRatherThanEchoed` and `testTheFirstRefusalOfARunIsAlwaysSent` |
| 911 | Two clients can hold the channel at once, which one connection per Space requires | pass (measured, live + gate) | First live run: a second connection completed its handshake and then never heard a packet, because connections ran on a **serial** queue and the first was parked in `read` for as long as its window was open. `InputServer` now uses a concurrent queue, one blocking read per connection. Re-measured: `second connection handshake: helloAck`. Pinned by `testFastInputChannelServesTwoClientsAtOnce` |
| 912 | A wrong session token is refused by the fast channel exactly as by the RPC socket | pass (measured, live + gate) | Live against the scratch worker: a correct-shape but wrong token got **no reply — the connection closed**, which is a refusal. `testFastInputChannelRefusesAWrongToken` asserts the same and fails if a `helloAck` ever comes back |
| 913 | Pointer capture takes the pointer on entry in Desktop Mode, keeps a gesture that leaves the picture, and restores the cursor on every exit | pass (unit, 16 tests) | `tests/Unit/PointerCapturePolicyTests.swift`, 16/16. The exits are the point: `releaseAll` is asserted from capture-with-button-held for both policies, and `CursorRectPolicy.imageRect` is asserted to be the **image** rather than the letterbox, and `nil` (not zero-sized) before the first frame |
| 914 | The fastest rate bug in the product was the default frame rate, not the input transport | pass (code + measurement on the same worker) | `DesktopViewerView`'s `@AppStorage("previewFPS")` default was **5**; the cursor is painted into the captured picture (`SCStreamConfiguration.showsCursor`), so the remote pointer could never be newer than a frame — up to 200 ms at that default, whatever the input path cost. The default is now **30**, `60` is offered, and a held button raises the running stream to 60 through `frame.setFPS` (`SCStream.updateConfiguration`, no reopen — a reconnect would rebuild the shared region and restart the decoder to move one number) |
| 915 | The frame-rate policy is expressed as relationships rather than constants, and pointer travel is independent of it | pass (unit, 12 tests) | `tests/Unit/InputRatePolicyTests.swift`, 12/12: idle is above zero and below the ceiling, active equals it exactly, a gesture is capped at 60, the ceiling is never exceeded, and a 5 FPS preference still sends travel at the display's rate |
| 916 | The pointer rate comes from the display rather than a constant, and the coalescer's old 30 Hz default is gone | pass (unit) | `DisplayRefresh.pointerRate` reads `CGDisplayCopyDisplayMode(...).refreshRate`, clamped to 30…120; `PointerTravelCoalescer`'s default is now `DisplayRefresh.defaultPointerInterval`, and `testTheCoalescerDefaultIsTheDisplayRateNotThirty` asserts it is *faster* than the rate it replaced (the old note said 30 Hz was "above what a preview at 5-15 FPS can show", and both halves of that sentence changed) |
| 917 | Raw pointer phases send the press while the button is still down, and the threshold model is still available where it is wanted | pass (unit, 7 tests) | `tests/Unit/InputRatePolicyTests.swift` `RawPointerPhaseTests`, 7/7. Raw mode sends `pointerDown` immediately with `NSEvent.clickCount`; the threshold path keeps its old behaviour (two points is a click, four starts a drag) because Fusion proxies still use it; a cancelled raw press emits its `pointerUp` so a window that closes mid-gesture does not leave the remote button held |
| 918 | `clickCount` rides the press, so a double-click stays a double-click | pass (code + additive-wire test) | `InputAction.pointerDown/pointerUp` gained `clickCount` (absent means 1, so every older client's meaning is unchanged — `testAMissingClickCountMeansOneClick` writes a zero count and gets 1). `InputSynthesizer` sets `mouseEventClickState` from it. Without this the window server sees the second press of a double-click as an unrelated first one |
| 919 | The window matcher accepts the windows the exact rule refused, and stays strict where a mistake is destructive | pass (unit, 12 tests) | §328 row 901 left this open. `WindowElementMatcher` scores title, frame IoU and stacking order; the tested cases are the ones that failed live — **no title**, **frame nudged past the old ±5 pt**, **no title in the catalogue** — plus a tie broken only by position. `decide(strict:)` drops the stacking term entirely and demands an exact title **and** frame, so `close`/`minimize`/`setFrame` still refuse an ambiguous guess while pointer input and raising do not |
| 920 | The worker's accessibility path uses the shared matcher rather than its own copy of the rule | pass (code) | `native/AgentSpaceWorker/Sources/AgentSpaceWorker/WindowActions.swift` observes every window of the pid (`Observation`) and asks Core for the verdict; the arithmetic is Core's, so it is testable with rectangles and no WindowServer. Raise and pointer take `strict: false`; close, minimize and set-frame take `strict: true` |
| 921 | The cursor's shape channel is claimed only after the worker measures that it can read a cursor in its own session | pass (code + claim asserted absent in a live handshake) | `CursorStateProvider` reads `NSCursor.current` through public AppKit only — no `CGSCurrentCursorSeed`, no SkyLight private API, per the product rule — and flips `shapesAvailable` after **two** successful shape reads. Until then the handshake does not advertise `cursorShapes` and `announceCapabilities` sends a second `helloAck` the moment it does. Measured: the live scratch worker's handshake carries no `cursorShapes` bit, and `testFastInputChannelEnforcesTheSameSessionRuleAsTheRPC` asserts exactly that, so a build that claimed it unconditionally would fail a gate |
| 922 | The local cursor is hidden with a scoped cursor rect and never with `NSCursor.hide()` | pass (code + unit) | `TransparentCursor` builds a 1×1 transparent `NSCursor`; the only place it is ever applied is `RemoteSurfaceNSView.resetCursorRects()`, scoped to the image rectangle, with `window.invalidateCursorRects(for:)` used for the transitions a still hand would otherwise miss. The process-wide hide count is never touched, so the failure mode of a missed reset is the **ordinary** cursor reappearing — never a Mac with no pointer. `CursorRectPolicyTests` pin the rect's extent |
| 923 | The viewer draws the agent's cursor itself, optimistically, and gives it up only on a real disagreement | pass (code + unit) | `RemoteCursorOverlayLayer` draws the position this client last *sent* and corrects to the worker's when the two differ by more than 2 pt — `correctionThreshold`. This is what takes the drawn cursor off the frame clock: the position the hand moved to is known locally the instant it moves |
| 924 | The capture stops painting a cursor exactly when the viewer draws one | pass (code + ordering) | `frame.setFPS` carries an optional `showsCursor`; `FrameClient.setEmbeddedCursor` is called from the one `onCursorChannelChange` transition, so the two cursors are mutually exclusive by construction. If the capability never arrives, `shapesAvailable` stays false, the stream keeps painting its cursor and the local one stays visible — the fallback §22 asks for, reached by doing nothing rather than by a second code path |
| 925 | Fusion drives the same shared channel with window-target packets, and keeps the RPC path for a worker that predates it | pass (code + unit) | `InputChannelRegistry` hands every surface of an account one connection with a use count; `FusionWindowController` sends `InputTarget.window(identity)` with **fractions**, which the worker resolves against the window's frame at apply time — `InputTargetResolution` is Core's, and `GestureFrameStore` keeps a drag on the frame its press named so a title-bar drag does not chase its own tail. `sendKey`/`fallback` still produce the old `window.input` shape for an old worker |
| 926 | The repo's own gates are green on this tree | pass (three of four layers; layer 4 recorded as not run) | `scripts/test.sh`: Swift **645 unit + 24 safety + 28 integration**, 697 tests, 0 failures. `packages/agentspace-mcp`: **24/24** Node tests. `scripts/updater-e2e.sh`: **19 checks passed**. `scripts/mcp-smoke.sh`: all assertions passed, including the fail-closed `SESSION_IS_CONSOLE` shapes. **Layer 4 did not run**: this machine has no attached agent account and a locked console, so `gui-verify.sh` reported 13 failures against a `dist/AgentSpace.app` that did not exist yet, and its session twin refused with `no app bundle` — exit 3, *nothing verified*, which is not a failure but is also not a pass. The live measurements above were taken with a scratch worker outside the product's own registry, which is why they hold without it |
| 928 | The Developer ID signing key the release needs was missing from the login keychain, and the first notarization attempt failed because of it | pass (resolved; recorded because the failure was real) | The first `scripts/notarize.sh` attempt with the only identity present (`Apple Distribution: Guofeng Liu (U8U443D7ZL)`) came back **Invalid** — asc submission `f230a23f-1e63-4cb8-8679-4c3d2d57745c`, `The binary is not signed with a valid Developer ID certificate` for all five Mach-Os. `security find-identity -v -p codesigning` offered no `Developer ID Application` private key in any keychain, and `asc certificates create --certificate-type DEVELOPER_ID_APPLICATION` is refused (`This operation can only be performed by the Account Holder`). The key existed elsewhere on this machine: `~/Desktop/clipnest-signing/DeveloperID.p12` holds `Developer ID Application: Guofeng Liu (U8U443D7ZL)`, serial **1A2A4844076D68ED**, `notAfter` 2027-02-01 — the same CN and team every earlier release was signed with. Its public key matches the `.cer` and the loose key byte for byte (`openssl` SHA-256 of both public keys: `eae31572…`). Imported into a **throwaway keychain** under `/tmp` (`security import … -T /usr/bin/codesign`, never the login keychain), which is removed at the end of this round. Nothing about the signature policy changed to make this work: the designated requirement a user's TCC grant is recorded against is untouched |
| 929 | 0.1.38 is notarized, stapled and Gatekeeper-clean, and both submissions were read back from Apple | pass (read back, not from the script's log) | Signed with `Developer ID Application: Guofeng Liu (U8U443D7ZL)` (team `U8U443D7ZL`, secure timestamp present), then `asc notarization list` — queried independently, not the script's own output — shows **`86f43e2b-950b-4871-96b6-d7f5405d944e` Accepted** for `AgentSpace-app-notarize.zip` and **`c6895e68-df30-4fa7-a924-20f7328a92da` Accepted** for `AgentSpace-0.1.38.dmg`. `stapler validate` and `spctl --assess` both accept the app; the DMG rebuilt from the **stapled** app carries a valid staple. `dist/AgentSpace-0.1.38.dmg` is **6,262,399** bytes, `sha256:2600ef02aff70aa96d93e2b8ae7ad2158d7b63a004e97eee438a1e00eabb01d5` |
| 930 | The gate passed all four layers on the artifact that was published | pass (measured on the gated bytes) | `scripts/check-all.sh`: the dist-integrity guard found the stapled app matching the stapled DMG's inner CDHash; layer 1 Swift **645 unit + 24 safety + 28 integration, 0 failures**; layers 2 `updater-e2e` (**19 checks**) and 3 `mcp-smoke` (all assertions) passed; layer 4 ran on the **console** this time — the screen was unlocked — and reported `gui-verify: 13 passed, 0 failed`, with `check-all: all 4 layers passed`. The bundle stamp reads `0.1.38 (432)` and `git rev-list --count HEAD` at `e944271` is **432** |
| 931 | The tag sits on the commit whose tree the bundle was built from | pass (measured) | `v0.1.38^{}` resolves to **`e944271`**, which is the code, test and documentation commit; the DMG was built from that tree with no later edit (the working tree was clean when `release.sh` ran, and `dist/` is gitignored). `0.1.37` was never published: `git diff v0.1.36 c31785d` is **four version strings and nothing else**, which the release notes say rather than implying a product change |
| 932 | The published asset is the byte-for-byte artifact the gate ran on, and it is what the product's own update channel offers | pass (measured against GitHub, three samples) | The release's `assets[].digest` is `sha256:2600ef02aff70aa96d93e2b8ae7ad2158d7b63a004e97eee438a1e00eabb01d5` over **6,262,399** bytes — identical to `shasum -a 256` of the local gated DMG. `GET /repos/misswell/AgentSpace/releases/latest` — the endpoint `SoftwareUpdater.latestReleaseURL` reads — answered `v0.1.38` on **three consecutive samples 20 s apart** with that same size and digest in its asset list, no `make_latest` write needed (§318 row 820's six-minute lag did not reproduce) |
| 933 | The release notes say the thing a person has to do, and say it first | pass (by construction) | The notes open with 「这一版要按两下，第二下才是让鼠标真正跟手的那一下」 and name both presses: 「检查更新」 for the viewer, 「重新安装助手…」 for the worker, with the sentence that says why — the input socket, the cursor provider, `frame.setFPS` and the window matcher are all in the worker. They also state the four things this round did **not** verify (no picture was ever looked at on a real desktop; the latency figure is a write cost, not pixel latency; CPU/RSS unmeasured) rather than letting the numbers imply more than they are |
| 927 | The harness override that made the safety suite usable does not weaken the readiness rule | pass (code) | `AGENTSPACE_DESKTOP_READY_TIMEOUT` bounds only the *startup* wait in `main.swift`; the readiness verdict itself is re-read on every input call (`Operations.input` step 1b, `InputConnection.gate`), so a session that comes up late still becomes usable without a restart. Effect measured: the safety suite went from **331.5 s** to **11.4 s** for the same 24 tests, all green |

## 331. Releases move to GitHub Actions, and the runner choice was a product fact (2026-09-23)

The owner's decision after 0.1.38: cut releases on GitHub Actions, the way
ClipNest, MacPilot, DevSweep and octo-shrink already do. The workflow calls this
repository's own `scripts/release.sh` and `scripts/notarize.sh` rather than
reimplementing them — a CI path that forked the release logic would release a
different product than the local flow does.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 934 | This repository had no CI before this round, so "the way we always did it" was the *other* repositories | pass (read, not recalled) | `.github/` did not exist and `git log --all --diff-filter=A -- .github/workflows/*` is empty; `gh secret list` and `gh workflow list --all` both answered nothing. ClipNest, MacPilot, DevSweep and octo-shrink each carry `APPLE_*` secrets and a `release.yml`; AgentSpace's 20-odd releases were all cut locally, which `docs/validation.md` records each time (including §321 row 838's local-signing fallback that shipped a bundle with no identity) |
| 935 | The first dry run failed, and the reason was the runner rather than the tree | pass (measured on the runner) | `macos-15` ran 697 tests and failed exactly **one**: `testDoctorReportsEveryCheckWithAStatus`, which asserts `doctor` exits 0 — and `Doctor.swift`'s second check is `version.majorVersion >= 26 ? .pass : .fail`, because the product requires macOS 26+ (plan §3). That is an environment verdict on a macOS 15 runner, not a defect in the change being released. The runner label is now `macos-26` (arm64), which exists and is what the product needs |
| 936 | The whole release job runs on a runner: sign, notarize, staple, verify what a user gets | pass (measured, dry run `35872277152`) | Both jobs succeeded. `security find-identity` resolved the imported `Developer ID Application: Guofeng Liu (U8U443D7ZL)` through the keychain search list; `scripts/release.sh` reported all five Mach-Os `ok`; `scripts/notarize.sh` submitted and Apple returned **Accepted** for both the app (`4dc9fe5a-c290-49eb-967d-1531abcb4eb4`) and the DMG (`835a54b7-f6e4-4b61-a84d-9f728e698972`) — read back from `asc notarization list`, not from the script's own output, on the same account that shows the earlier local submissions. `stapler validate` passed for the app, the DMG **and** the app inside the mounted DMG, whose CDHash was asserted equal to the outer app's |
| 937 | The gate on the runner is layers 1–3, and layer 4's absence is stated rather than implied | pass (measured) | Layer 1 `scripts/test.sh`: **697 tests, 0 failures** in 58 s. Layer 2 `updater-e2e`: **19 checks**. Layer 3 `mcp-smoke`: all assertions, including the fail-closed `SESSION_IS_CONSOLE` shapes. Layer 4 was **not run**: `gui-verify.sh` needs a console Aqua session with an Accessibility grant, which a hosted runner cannot be given. The gate job probes System Events first and records the gap in the run's job summary, and the publish step appends the same sentence to the release notes — so a CI release never claims a layer it did not run |
| 938 | A dry run cannot publish, and the live release was untouched by it | pass (measured) | `inputs.dry_run` gates the publish step; it reported `skipped`. After the run, `gh release list` still showed the same three releases and `releases/latest` still answered `v0.1.38` with the **local** DMG's digest `sha256:2600ef02…` over 6,262,399 bytes — not the CI build's `53e34109…` over 6,213,603 bytes. The two differ because a DMG is not byte-reproducible across machines (timestamps, ordering); that is why the published asset for a release is the one its own gate measured, and why the publish step asserts the channel's digest matches |
| 939 | Seven repository secrets are set, and the p12 travels byte-identically | pass (measured) | `gh secret list` shows `APPLE_CERTIFICATE_P12`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_DEVELOPER_ID`, `APPLE_TEAM_ID`, `ASC_KEY_ID`, `ASC_ISSUER_ID`, `ASC_PRIVATE_KEY`. The base64 the workflow decodes with `base64 -D` was round-tripped locally first and compared with `cmp`: **identical** `sha256:937afc01…`. The notary credential is the App Store Connect API key (`5KDZSQND5Q` / issuer `102e47ab-…`), proved against `notarytool` itself with a live `store-credentials` + `history` before it was ever used — `scripts/notarize.sh` verifies its profile the same way, and a profile *name* proves nothing |
| 942 | **The first release cut entirely by CI is published, and a user-side check of the published bytes passes** | pass (measured, runs `35875629950` and `35874571287`) | `v0.1.39` is a version-only release whose purpose is exactly this proof. All three jobs green: `validate` (version strings, tag, notes) in seconds, `gate` (Swift **697/697**, `updater-e2e` **19 checks**, MCP smoke), and `release` — Developer ID signed, both Apple submissions **Accepted**, app/DMG/inner-app staples valid, Gatekeeper accepts. The publish step asserted `releases/latest answers v0.1.39 (attempt 1)` and `channel digest: sha256:28ac99d1…`, then the artifact was pulled back and checked the way a user's Mac would: **6,213,665** bytes, digest **identical** to the runner's, `stapler validate` on the app **inside the mounted DMG** worked, `spctl` answered `source=Notarized Developer ID` / `origin=Developer ID Application: Guofeng Liu (U8U443D7ZL)`, and the bundle reads **0.1.39** |
| 943 | Two real defects in the workflow were found by running it, not by reading it | pass (measured, and both fixed) | (a) **The first tag-triggered release failed after notarizing.** `validate`, `gate`, `build+sign` and `notarize` all succeeded — both submissions Accepted — and then the verify step failed with `missing dist/AgentSpace-.dmg`. Cause: a blanket rewrite of `steps.release.outputs.*` → `needs.validate.outputs.*` had also hit the `validate` job's own `outputs:` block, turning it into a self-reference that resolves to empty. The job outputs now come from that job's own steps. (b) **The `validate` job never actually compared the tag on its first run.** It called `plutil`, which does not exist on `ubuntu-latest`, so it exited **127 (command not found)** — a check that failed for the wrong reason, which is worse than no check because it looks like it worked. One line of `python3 -c plistlib` replaced it, and the step was then exercised locally in five modes: mismatched tag refused naming the tag, matching tag accepted, dispatch path accepted, missing notes file refused, and a bumped CLI version caught in all three other files by name |
| 941 | The tag trigger fires, and a mismatched tag is refused before the gate rather than after it | pass (measured twice, the second time on the fix) | First measurement: pushing `v0.1.999` against a tree reading 0.1.38 did trigger the workflow (`event=push ref=v0.1.999`) and the guard did fire — `##[error]Tag v0.1.999 does not match the tree's version v0.1.38` — but the **whole gate job ran first** (~4 min), because the check lived in the release job. Tag, version strings and the notes file now have their own `validate` job on a cheap runner. Second measurement, same mismatched tag: `Validate=failure Gate=skipped Sign=skipped` in **~20 s**. Both test tags were deleted from origin and locally; `v0.1.38` is still the newest tag |
| 940 | Release notes stay authored, and required | pass (by construction) | The workflow fails with a named `::error::` when `docs/releases/<tag>.md` is missing. This exists because the notes carry a rule `--generate-notes` cannot satisfy: say what a person has to do, first. For 0.1.38 that is the two-press rule, and a commit log would have buried it. `docs/UPDATE_CHANNEL.md` documents the convention and the seven secrets, including why the notarization credential is an API key rather than an Apple ID |

## 332. Desktop clicks survive an older Worker, and Worker versions reconcile automatically (2026-09-24)

The reported symptom was a click inside 「打开桌面」 doing nothing. The installed App
was 0.1.39 and the running Worker was 0.1.36. The Worker runtime contained
`worker.sock` and `frame.sock`, but no `input.sock`; the new viewer sent every
pointer gesture only through `InputClient`, whose unready state silently dropped
it. The old `input` RPC still answered `click AgentUse 960 540 --json` with
`performed: 1` in the agent account's background session. A second defect was
visible in the new channel's code: `readLoop` waited indefinitely for a packet on
the same serial queue used to write pointer packets.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 944 | The reported click failure is explained by an actual installed-version mismatch | pass (measured) | `/Applications/AgentSpace.app` reports **0.1.39**; `/Library/Application Support/AgentSpace/Worker/active/agentspace-worker` reports **0.1.36**. AgentUse is `Ready`, `acceptsInput=true`, `onConsole=false`, and its runtime has no `input.sock` while `worker.sock` and `frame.sock` exist. `InputClient.sendForKind` used to return silently with no socket; `DesktopViewerInput.send` had no pointer RPC fallback. This is the active failure path, separate from §330's new-channel queue bug. |
| 945 | A viewer over a pre-channel Worker can still click, drag and scroll through the original RPC | pass (code and old-Worker RPC measured; viewer acceptance pending) | `LegacyDesktopPointer` converts raw down/up into an atomic `.click`, raw travel into `.drag`, and carries double-click count and scroll anchor. Its three tests pass. `DesktopViewerInput` selects the original RPC while the channel is unready and keeps a press on one transport through release; the actual 0.1.36 Worker answered an RPC click in AgentUse's session with `performed: 1`. The app-side viewer was not clicked on a live installed 0.1.40 bundle yet. |
| 946 | A quiet reader cannot indefinitely hold a click behind it | pass (red/green) | `InputChannelWorkQueuesTests.testAQuietReaderDoesNotStopAClickFromBeingWritten` held a reader and demanded a write in 0.5 s. With the old single-queue arrangement reproduced as `reader = writer`, it failed after 0.624 s with the named unfulfilled click expectation; with separate read/write queues it passed in 0.000 s. `InputClient` now uses that queue pair and aborts the original socket on disconnect. |
| 947 | Version mismatch starts one automatic typed Worker repair, with no refresh prompt loop | pass (policy tests and code; automatic privileged install acceptance pending) | `AutomaticWorkerUpdatePolicyTests`: 0.1.36 vs 0.1.40 attempts once; a repeat is ignored; a newly observed 0.1.38 can attempt; a matching 0.1.40 clears the record; unknown version triggers nothing. `AppModel.reload` checks each answering Worker's `hello` version off the main actor and calls its existing `prepareWorkerForAuthorization`, which uses the closed helper operations and waits for the new version. A failed attempt is shown through `lastError`. Only a copy in `/Applications` attempts this automatically; debug and dist copies do not replace the installed Worker. |
| 948 | Local source and isolated UI checks pass before packaging | pass (measured on the tag candidate) | Swift full suite **703/703**, MCP tests **24/24**, MCP smoke all assertions, updater-e2e **19/19**. A temporary debug bundle at `/tmp/agentspace-0140-bundle/AgentSpace.app` passed **13/13** GUI checks inside AgentUse's session without signalling the owner's app. The final Swift and UI runs include the `/Applications` auto-update guard. This GUI gate does not click through a live desktop viewer; that acceptance remains separate. |
| 949 | 0.1.40 is published and is the asset the in-app update channel serves | pass (read back) | `v0.1.40^{}` resolves to **8d96184**, the tested code and release-notes commit. Actions run [35940848637](https://github.com/misswell/AgentSpace/actions/runs/35940848637) passed all three jobs. Its log reports Apple notarization **Accepted**, valid staples for App and DMG, and Gatekeeper acceptance; the publish step asserted `releases/latest answers v0.1.40 (attempt 1)` and the channel digest. Independent `GET /repos/misswell/AgentSpace/releases/latest` answered a non-draft, non-prerelease v0.1.40 with `AgentSpace-0.1.40.dmg`, **6,253,135** bytes and `sha256:f673af39a78eaaa0af4405032148d0b1f245f8f3612cc2ce2523dd475898440d`, equal to the CI-gated DMG digest. The CI runner cannot run the Aqua/TCC GUI gate; local session-side 13/13 is row 948. |

## 333. Fusion proxy resize changes the actual app window (2026-09-24)

The reported behaviour was that dragging a standalone Fusion window changed only
its local mirror. The proxy capped its initial size at 1200×900 points, imposed a
420×300 minimum, and never called the existing `window.setFrame` worker method.
The rendered image could therefore be letterboxed while fractional input still
addressed the original remote geometry.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 950 | A proxy's initial content size follows the actual app window up to the host screen's visible area, and its first frame request uses the host display's backing pixels | pass (code and build) | `FusionWindowController` removes the 1200×900 cap and autosaved proxy size, uses the remote frame with the host screen's visible bounds, and multiplies the first capture request by `backingScaleFactor`. `RemoteSurfaceNSView.layout` continues to reconfigure capture to its backing-pixel bounds. Swift build passed on this source. |
| 951 | Dragging a proxy's edge asks the worker to resize the actual window, preserving its current position and accepting app-imposed size constraints | pass (code and compile; live resize acceptance pending) | `windowDidResize` coalesces requests and `windowDidEndLiveResize` sends the final size. `SpaceService.windowSetSize` sends the existing protocol method with width/height. The worker resolves the current CG window by identity and preserves its current x/y when absent; `WindowActions.setFrame` strictly resolves the AX element, changes its size, and returns the measured AX frame. The proxy adopts a size the app actually accepted. A live drag against the installed 0.1.41 Worker has not yet been observed; the older installed Worker cannot answer the new size-only response. |
| 952 | Local build and isolated UI gate pass | pass (measured) | Final `swift test`: **705/705**, zero failures. `npm test` **24/24**, `scripts/mcp-smoke.sh` all assertions, and `scripts/updater-e2e.sh` **19/19** pass. A temporary debug bundle at `/tmp/agentspace-0141-bundle/AgentSpace.app` passed **13/13** `session-gui-verify.sh` checks in AgentUse's background Aqua session; none of the owner's processes or windows were signalled. The UI gate covers the App shell, not the new live Fusion resize. |
| 953 | App launches and background window discovery do not create local proxies | pass (code and build; interaction acceptance pending) | `FusionManager.fuse` now only launches or activates the App. `FusionSession.reconcile` only maintains proxies the person already opened; `FusionWindowPickerView` lists windows read-only and invokes `openWindow` only from its explicit per-row button. The toolbar's 「打开应用」 opens that picker. Swift build and localization tests pass; the live picker-to-proxy path has not yet been clicked in an installed build. |

## 334. The Desktop Viewer fits its window to the whole desktop (2026-09-24)

The supplied screenshot shows a 1920×1080 agent desktop centred in a much
taller viewer, leaving black strips above and below. AgentUse's current
`CGDisplayCopyAllDisplayModes` call returns **only 1920×1080**. A short lived
`CGVirtualDisplay` created in AgentUse's session did support another ratio, but
the owner session immediately reported it as a third online display (id 27),
then returned to two displays when the probe exited. The owner rejected that
global layout change and chose an aspect preserving viewer with no bars or crop.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 954 | The current agent display cannot directly assume an arbitrary viewer aspect, and a virtual display is not session isolated | pass (measured) | `agentspace exec AgentUse` ran `CGDisplayCopyAllDisplayModes`: current and sole mode **1920×1080**. A reversible 12 second virtual display probe inside that account created **1200×800 display 27**. The owner's `CGGetOnlineDisplayList` immediately changed from two to three displays and returned to two on probe exit. No display modes or main-screen settings were changed. |
| 955 | The desktop viewport, excluding viewer controls, stays at the source display's aspect without crop or bars | pass (geometry tests and build; live viewer acceptance pending) | `DesktopViewerWindowController.windowWillResize` derives the viewport from the proposed content size after subtracting its measured header/footer/dividers, then adjusts height to `width × displayHeight / displayWidth` and caps the result to the host screen's visible height. `DesktopViewportSizingTests` covers the reported tall 1532-point viewer and a screen-height cap; **2/2** pass. The stream and input mapping remain aspect-preserving. A live drag of the installed viewer is not yet measured. |

## 335. Desktop Viewer recovery and honest backing pixels (2026-09-24)

After 0.1.41 installed, the owner reported that switching zoom left desktop
clicks unresponsive and that the Retina resolution still looked wrong. The
supplied 0.1.40 screenshot showed a stale 「休眠」 state and a `desktop not ready`
input refusal; the currently installed App and Worker both report 0.1.41, and
the Worker now answers `status` with `Ready`, `acceptsInput=true`. A direct
`agentspace click AgentUse 960 540 --json` answered `performed: 1` while the
desktop was in that ready state. The GUI's status had no live refresh while a
detached viewer remained open, and its refusal sink ignored the channel's nil
value after successful input.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 956 | 0.1.41 shipped before the new report and cannot be reused as a patch tag | pass (read back) | `v0.1.41^{}` resolves to `8bbe015`; Actions run [35943611102](https://github.com/misswell/AgentSpace/actions/runs/35943611102) completed all three jobs. `GET /repos/misswell/AgentSpace/releases/latest` answered `v0.1.41` with `AgentSpace-0.1.41.dmg`, **6,289,192** bytes and `sha256:8ab509b98180e6aff97c185be138686c9071afea6a91d66bd01194d510c228e9`. The installed App and Worker both answer 0.1.41. |
| 957 | The detached viewer can recover input after a background session wakes, and a successful packet clears a prior refusal | pass (code and compile; installed click acceptance pending) | `DesktopViewerView` now awaits a three-second per-account status refresh only while open. `AppModel.refreshViewerStatus` performs the lightweight Worker status off the main actor, preserves resource data, and refuses to overwrite a newer snapshot. `DesktopViewerInput` observes optional `lastRefusal` values, including nil; before this change `.compactMap` discarded nil forever. Source builds. A live post-zoom click on the installed patch is still required. |
| 958 | Zoom cannot invent Retina detail or ask for a buffer beyond the source pixel limit | pass (unit test and measured source geometry) | `RemoteSurfaceNSView.layout` uses `CaptureSizing.viewerRequest`, applying the capture limit after magnification. `testViewerZoomNeverUpscalesPastTheSourcePixels` proves 200% over a 1920×1080 source requests no more than **1920×1080**, rather than a fake 3840×2160 buffer. `CGDisplayCopyDisplayMode` in AgentUse reports the main display as **1920×1080 points / 1920×1080 pixels (1×)**; the second built-in display reports **1512×982 points / 3024×1964 pixels (2×)**. `CGDisplayCopyAllDisplayModes` for the main display, including duplicate low-resolution modes, yielded only the 1× mode. The footer now shows logical points, physical source pixels and scale separately; it does not relabel an upscale as Retina. |
| 959 | The local 0.1.42 candidate passes automated and separate-session checks; a stale AgentSpace copy in that session can invalidate the UI instrument | pass (measured) | `swift test` **706/706**, `npm test` **24/24**, MCP smoke all assertions, updater E2E **19/19**. The first session GUI run found no subject window; the second found only a menu shell. The same symptom reproduced against installed 0.1.41. `agentspace apps AgentUse --json` revealed a still-running `/private/tmp/agentspace-0141-bundle/AgentSpace.app` with the same bundle ID; after terminating only that AgentUse-owned test copy, `scripts/session-gui-verify.sh --account AgentUse --bundle /tmp/agentspace-0142-bundle/AgentSpace.app` passed **13/13**. This gate checks the App shell, not a live post-zoom click. |
| 960 | The notarized 0.1.42 artifact is the asset the update channel serves | pass (release read back and local bytes) | Annotated `v0.1.42^{}` resolves to `7eaa87a0375ed9d320f3e6ebbe7d32e0e40d70ca`. [Actions run 35947723915](https://github.com/misswell/AgentSpace/actions/runs/35947723915) completed all three jobs successfully. `GET /repos/misswell/AgentSpace/releases/latest` answers `v0.1.42`, not a draft or prerelease, with one `AgentSpace-0.1.42.dmg` asset of **6,304,420 bytes** and `sha256:934b7806d67ae079b05afa994b913f8467aef8d09ae430fc71a9654a0d307474`. A downloaded copy at `/tmp/agentspace-0142-release/AgentSpace-0.1.42.dmg` has the same byte count and SHA-256, and `xcrun stapler validate` reports `The validate action worked!`. Release: [v0.1.42](https://github.com/misswell/AgentSpace/releases/tag/v0.1.42). |

## 336. Real 2× desktop and complete window-button clicks (2026-09-24)

The owner required a minimum 2× desktop and then reported that an App's red
close or yellow minimise button reacted to a click but did not act. The agent
session's current main display is genuinely 1×. Its second online display is
2×; selecting that display changes which desktop area the viewer shows but
does not change the owner's display layout. Existing App windows on the 1×
area are not moved automatically.

| # | Claim | Verdict | Evidence |
|---|---|---|
| 961 | A real 2× capture source exists in AgentUse, with a nonzero origin that input must account for | pass (measured) | In AgentUse, `CGGetOnlineDisplayList` returned main display **23** at `(0,0)`, 1920×1080 points/pixels, and display **22** at `(1920,98)`, 1512×982 points / **3024×1964** pixels. `agentspace screenshot AgentUse --display 2 --json` captured a **3024×1964** PNG but the old reply claimed the *main* display's 1920×1080 backing pixels and 1× scale. The Worker now publishes `retinaDisplay` from actual modes, the viewer requests that display ID and sends local input to a new typed display target; the Worker validates local coordinates and adds the display origin before posting. Screenshot metadata uses the selected display's mode. |
| 962 | A stationary click was incorrectly emitted as down, drag, up | pass (red/green regression) | `swift test --filter RemotePointerGestureTests/testRawClickDoesNotInsertADragBeforeMouseUp` first failed: `draggedPhases` produced a `pointerDrag` on the release path even when the hand did not move. After the fix, the same test passes; `releaseTravelPhases` also suppresses a one-point release jitter while retaining real drag phases. The user's live close/minimise click still needs verification on the installed patch. |
| 963 | The local 0.1.43 candidate passes the repo's executable checks | pass (measured, with live Desktop Viewer acceptance pending) | `swift test` **709/709**; `npm test` **24/24**; `scripts/mcp-smoke.sh` all assertions; `scripts/updater-e2e.sh` **19/19**. `scripts/session-gui-verify.sh --account AgentUse --bundle /tmp/agentspace-0143-bundle/AgentSpace.app` passed **13/13** in the agent session. That instrument checks the App shell, not a 2× live stream or the installed Worker's click path; those still require the 0.1.43 App and Worker in the actual session. Re-run the same day after the authoring session was cut off: all four executable checks answered green again, and the session gate first **failed twice** (build stamp, registry notice and the Settings window all absent from the AX tree the checker read). A hand-written AX probe over the same `agentspace exec` channel launched the same bundle and read the complete tree — `appBuildVersion` = `构建 0.1.43 (447)` and the registry notice present within ~1 s of launch — so the subject was whole while the instrument saw it empty; the identical checker binary then passed **13/13** both run directly through `exec` and through the script. The two earlier verdicts stand in this table as instrument flaps, mechanism unproven, not build verdicts. |
| 964 | The notarized 0.1.43 artifact is the asset the update channel serves | pass (release read back) | [Actions run 35952783573](https://github.com/misswell/AgentSpace/actions/runs/35952783573) completed all three jobs successfully (validate; gate layers 1–3; sign/notarize/publish). Annotated `v0.1.43^{}` resolves to `3cd1c1d22f2cbb4adc1257ae83a3232ff0b5b681`. `GET /repos/misswell/AgentSpace/releases/latest` answers `v0.1.43`, not a draft or prerelease, with one `AgentSpace-0.1.43.dmg` asset of **6,321,113 bytes** and `sha256:42eeb5d8034ae7e4fa6a917de10e7f5d21ee4c7ec2e4563e5c4483df72c602da`. Release: [v0.1.43](https://github.com/misswell/AgentSpace/releases/tag/v0.1.43). Live close/minimise clicks and the 2× capture on the installed pair remain unmeasured (row 962's open half). |
| 965 | A pinned display id goes stale, and every click is refused with it | pass (reproduced live, then fixed and re-measured) | At 14:04 the owner reported clicks dead on the released 0.1.43. A scratch probe speaking the viewer's exact fast-channel packets reproduced it against the installed worker: a pointer packet targeted `.display(22)` answered `invalidTarget — the requested Retina display is no longer available`, because the session's 2× display now enumerated as **58** (`status.retinaDisplay.id`), the id having changed since 0.1.43's own §336 rows 961–963 measurements. A click targeted at the current id 58 succeeded (`status=ok applied=(2087.8, 182.6)`), and the local tracker, worker synthesis and app client were all ruled out by reading them before touching them. The fix replaces the pinned id with a role: a new `InputTarget.retinaDisplay` (wire kind 3, no payload) is resolved by the worker **per packet** through `ScreenCapture.retinaViewerDisplay()` — the same source status publishes — with the existing coordinate validation and origin arithmetic; the frame open re-resolves a vanished id to the same role instead of failing. Live re-measurement with the fixed worker: the role-targeted click answered `ok applied=(2087.9, 182.1)`, a listen-only `CGEventTap` in the agent session recorded exactly one `DOWN` + one `UP` at `clickState=1` (no doubling), and the TextEdit close button clicked this way really closed the window — the live half row 962 left open. |
| 966 | The 0.1.44 candidate passes the repo's executable checks; the session Dock facts are recorded | pass (measured) | `swift test` **710/710**; `npm test` **24/24**; MCP smoke all assertions; updater E2E **19/19**; session GUI gate against the 0.1.44 bundle **13/13**. The agent session's Dock process was found **not running** at all this afternoon (started manually at 14:25); macOS renders the Dock only on the session's main 1× display — enabling `spaces-display-has-separate-spaces` and restarting the Dock did **not** put one on the 2× display (measured, then reverted) — so a 2× desktop view has no Dock by the system's own arrangement, not by a capture gap. |
| 967 | The notarized 0.1.44 artifact is the asset the update channel serves | pass (release read back) | [Actions run 35967174749](https://github.com/misswell/AgentSpace/actions/runs/35967174749) completed all three jobs successfully. Annotated `v0.1.44^{}` resolves to `f9aa34f5ce1a87a320b287ab7b49c58123f24c53`. `GET /repos/misswell/AgentSpace/releases/latest` answers `v0.1.44`, not a draft or prerelease, with one `AgentSpace-0.1.44.dmg` asset of **6,324,312 bytes** and `sha256:239b3c844479194182888d1c8ed151de7cf21a552a95e8d1f93445b14b5ee27f`. Release: [v0.1.44](https://github.com/misswell/AgentSpace/releases/tag/v0.1.44). |
| 968 | A worker process outside launchd survives an install+start and keeps serving the old version | pass (reproduced live, then repaired and hardened) | The owner reported the 0.1.44 worker update failing with no authorization prompt. Measured: the app's automatic path had **succeeded at the file level** — `versions/0.1.44` installed and `active` swapped at 15:13, no admin prompt because none is needed for a worker swap — but the serving process (pid 21433, started 14:52) was an orphan started with `nohup` during the day's click experiments, outside launchd, still holding the runtime lock, so launchd's replacement died with "another worker is already serving this agent" and the old binary kept answering 0.1.43. Repaired in place: the orphan was terminated and the worker re-adopted by launchd (`--quiet` invocation, pid 44421 at 15:18), which answered the 0.1.44-only role target (kind 3) while a pre-0.1.44 worker would have refused the kind byte. Hardening shipped in 0.1.45: `workerControl(start:)` now terminates processes of this uid whose command line runs under the helper's own worker paths after the bootout, parsed by the tested `HelperCommand.unmanagedWorkerPids`. Local: `swift test` **713/713**; `npm test` **24/24**; MCP smoke all assertions; updater E2E **19/19**; session GUI gate on the 0.1.45 bundle **13/13**. |
| 969 | The notarized 0.1.45 artifact is the asset the update channel serves | pass (release read back) | [Actions run 35970051653](https://github.com/misswell/AgentSpace/actions/runs/35970051653) completed successfully. Annotated `v0.1.45^{}` resolves to `5651349` (the commit the tag's tree was recorded against; the tag was cut one docs fix before this final commit). `GET /repos/misswell/AgentSpace/releases/latest` answers `v0.1.45` with one `AgentSpace-0.1.45.dmg` asset of **6,339,830 bytes** and `sha256:cf92df630f6db259491e4b28a6f166ad11697120e4e64743694e1cd008c4ee04`. Release: [v0.1.45](https://github.com/misswell/AgentSpace/releases/tag/v0.1.45). |

## 337. One logical 2× Retina main desktop for the remote viewer (2026-09-24)

The owner required exactly one screen in the remote desktop, and it had to be
the real 2× Retina panel as the *main* display — not a viewer pinned to the
second screen (which is why the Dock, rendered only on the session's main
display per row 966, never appeared). The same session also hit clicks acting
as double clicks: the menu opened and collapsed at once. Both fixes are here,
and the display switch lives in the worker's stream lifetime rather than in a
test process. The live instrument is a scratch worker built from this tree
running in AgentUse's own session, driven by
`tests/probes/RetinaDesktopProbe.swift` (a frame-open RPC, a real ACK-ing
frame reader, cursor-position reads and CG display facts). Scratch displays
that day: main **83** 1920×1080 pt/px 1× at `(0,0)`; built-in 2× display **82**
1512×982 pt / 3024×1964 px at `(1920,98)` — the id had drifted again
(22 → 58 → 82), which is exactly why nothing in this round pins an id.

| # | Claim | Verdict | Evidence |
|---|---|---|---|
| 970 | The viewer's stream is one logical 2× Retina main desktop, applied by the worker for the stream's lifetime and restored at the last close | pass (measured live in the agent session) | `frame.open` with `{"kind": "retinaDesktop"}` against the scratch worker answered ok, and the probe's in-session facts read: main **82** at `(0,0)` 1512×982 pt / **3024×1964** px, display 83 `mirrorSource=82`, `active=1`, marker `retina-desktop-layout.json` present; the first ACK'd frame was **3024×1964**. `frame.close` → main **83** at `(0,0)` 1920×1080 1×, 82 back at `(1920,98)` unmirrored, `active=2`, marker absent. The main-display role moving to the 2× panel is the mechanism row 966 measured as "the Dock renders only on the session's main display", so the Dock arrives with the flip without any Dock-specific code. |
| 971 | A worker killed mid-stream cannot strand the arrangement: its replacement recovers it from the marker | pass (reproduced live; first design failed for a measured reason, then redesigned and re-measured) | `kill -9` of the worker mid-stream left the session mirrored with the marker present (nobody left to restore it — the intended interrupted state). The first replacement restored the *visible* arrangement but kept the marker, so recovery had bailed. Two measured causes, both in the resolution path: (1) `CGDisplayCopyAllDisplayModes` answered **0 modes** for the 2× display both while it was a mirror source and after it un-mirrored (macOS 27), and a single mode for the other display — so re-finding a saved mode by enumeration can never work here; (2) `CGMainDisplayID()` immediately after a *successful* `CGCompleteDisplayConfiguration` still named the mirror source — the main reassignment settles late, which read as a failure and left the marker. The redesign saves and restores only the **arrangement** (un-mirror + origins + main): `acquire` never changes a mode, and a display's own mode returns when it leaves the mirror set (measured: 83 came back at 1920×1080 by itself), so no mode is re-applied. `waitForMain` bounds the main check at 6 s. Re-measured end to end: interrupted state reproduced → replacement worker started → main back to 83 at `(0,0)` 1×, 82 at `(1920,98)` unmirrored, `active=2`, **marker removed**. |
| 972 | A client that disappears mid-stream releases the layout through its connection's end | pass (measured) | The probe holds each open stream only while it runs and exits without `frame.close` (the frame server's `defer { manager.close(...) }` fires on socket EOF). After the probe's timer exit with the worker still running, the session read main 83, `active=2`, marker absent — the same release path a crashed or force-quit viewer takes. |
| 973 | Input coordinates are bounded by whichever main display the layout has applied, and a click is still exactly down + up | pass (measured with cursor positions and acks) | With the stream open (main = 82, 1512×982): a `.desktop` move to (1400,900) was accepted — the worker's own cursor packet then read (1400,900); a move to (1600,900) was acked `invalidCoordinate — coordinate (1600.0, 900.0) is off the main display (1512x982 points)`; a click at (1450,940) answered `ok` on down and `ok` on up, no second down. After the last stream closed (main = 83, 1920×1080): the same (1600,900) was accepted and the cursor packet read (1600,900); the out-of-bounds probe (9999,9999) was refused with the message now naming `1920x1080 points` and `scale=1`. The bound moves with the layout because both the validation and the viewer's `.desktop` packets read the same main-display geometry. |
| 974 | A raw press that reaches the session can never be answered by a second complete click, and the fix does not reintroduce the synthetic release drag | pass (red/green, two pinned tests) | The owner's symptom is the mid-press downgrade: capture/cursor state re-resolves between down and up, `rawPhases` drops, and `endedPress` took the threshold path that answers `.click` — a second full click after the down already went out (「按下、再次按下、抬起」 in the worker log). The fix marks the press `streamStarted` at down time, so the release is an up even after `endControl()` (`testRawPressDoesNotBecomeASecondClickWhenControlEndsBeforeRelease`). The partial fix alone was **red**: `releaseTravelPhases` keyed its zero-distance suppression on the same flag, so every stationary raw click gained a synthetic 1-point `pointerDrag` before the up — `testRawClickDoesNotInsertADragBeforeMouseUp` failed, exactly the defect 0.1.43's row 962 paid for. Final form splits the flag: `streamStarted` (a down went out) versus `travelStreamed` (a travel phase went out), and the suppression keys on the latter. Full `swift test`: **715/715**. |
| 975 | A viewer rebuild does not flip the agent session's arrangement | pass (code) | `restartPreviewIfNeeded` opens the replacement stream *before* stopping the old one, so the worker's user count never reaches zero between the two and no restore-then-reapply happens on a resize or quality change. The status-poll restart is keyed on the display's id and pixel shape (`retinaDesktopRestartKey`), deliberately not on its origin: the origin moving to (0,0) is what the stream itself applies, and restarting on it would tear the picture down seconds after every open. |
| 976 | An old worker refuses the new target cleanly — additive wire, protocol 1 unchanged | pass (measured against the installed 0.1.45 worker) | The same `frame.open {"kind": "retinaDesktop"}` request sent over the installed worker's real socket answered `BAD_REQUEST: frame target kind must be display or window` — a typed refusal, no crash, no layout change, no state left behind. Display-id and window targets decode unchanged, so old apps keep working over a new worker and a new app over an old worker degrades to this visible error until 「重新安装助手…」. |
| 977 | The 0.1.46 candidate passes the repo's executable checks, and its notarization is read back from Apple | pass (measured) | `swift test` **715/715**; `npm test` **24/24**; `scripts/mcp-smoke.sh` all assertions; `scripts/updater-e2e.sh` **19/19**; `scripts/check-all.sh` **all 4 layers** — the console instrument answered exit 3 (the owner's own app, pid 70001, was running and was not touched) and its session-side twin answered **13/13** in AgentUse's session against the notarized dist bundle. `asc notarization list`: app submission `cffc01a2-7c36-4329-90ce-5c684d373711` **Accepted**, DMG submission `f5dbc9c1-e821-47aa-a711-928c4eee3d6e` **Accepted**; stapled and Gatekeeper-clean (`dist/AgentSpace-0.1.46.dmg`). |
| 978 | The published 0.1.46 artifact is the asset the update channel serves | pass (release read back) | [Actions run 35987554350](https://github.com/misswell/AgentSpace/actions/runs/35987554350) completed successfully (validate; gate layers 1–3; sign/notarize/publish). Annotated `v0.1.46^{}` resolves to `c1929e5`, the commit whose tree the runner built. `GET /repos/misswell/AgentSpace/releases/latest` answers `v0.1.46`, not a draft or prerelease, with one `AgentSpace-0.1.46.dmg` asset of **6,358,314 bytes** and `sha256:e42eb983d0063a1c5e9570427aa303a1942b6a228fd785953a892f2000302ef5` — the same digest a fresh `curl -L` of the download URL hashes to, byte for byte. Release: [v0.1.46](https://github.com/misswell/AgentSpace/releases/tag/v0.1.46). What the release notes say is still owed on a person's hands: GUI-gesture acceptance (drag, scroll feel, double-click timing) on the installed pair. |
