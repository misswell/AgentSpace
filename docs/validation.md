...
| 580 | An updated worker's disk measurement produces no gate activity in the attached account | pending | to be re-measured with `/usr/bin/log show --predicate 'process == "tccd"'` after the worker update; the 568 evidence above is the before picture |
