...
| 726 | The release does not claim the frame engine's real-machine gates | pass (disclosed in the product's own language) | §304 row 687 and §306 rows 704/710/714 stay open and the release notes say so: the installed worker on this Mac is still 0.1.13, which answers `preview --stats` with `unknown method 'frame.stats'` (§306 row 712), so no latency/CPU/bandwidth/static-desktop/fast-user-switch number exists for this code. The notes also carry plan §14's other half — an update replaces only `/Applications/AgentSpace.app` and never the helper or the installed worker, so the new frame path needs the explicit 「重新安装助手…」 action, and 「助手版本过旧」 afterwards is the correct state (§302 row 666, helper-stale memory) |
