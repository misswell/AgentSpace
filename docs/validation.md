...
| 928 | **The release is built but not published, and the reason is one missing key rather than a failing gate** | blocked (recorded, not worked around) | `AGENTSPACE_CODESIGN_IDENTITY="Apple Distribution: Guofeng Liu (U8U443D7ZL)" scripts/release.sh` produced `dist/AgentSpace-0.1.38.dmg` (6,259,080 bytes) with all five nested binaries verifying, team `U8U443D7ZL`, secure timestamp present — and `scripts/notarize.sh`'s asc submission **`f230a23f-1e63-4cb8-8679-4c3d2d57745c`** came back **Invalid**: `The binary is not signed with a valid Developer ID certificate` for all five Mach-Os. The cause is on this machine, not in the tree: `security find-identity -v -p codesigning` offers only *Apple Development* and *Apple Distribution* — no `Developer ID Application` private key exists in any of the three keychains (17 private keys checked by label). The Apple account **does** hold two `DEVELOPER_ID_APPLICATION` certificates for this same team and CN, so the designated requirement a user's TCC grant is recorded against (`identifier "com.agentspace.AgentSpace" and anchor apple generic and certificate leaf[subject.CN] = "Apple Distribution: Guofeng Liu (U8U443D7ZL)"`) is unaffected; what is missing is the private half. `asc certificates create --certificate-type DEVELOPER_ID_APPLICATION` is refused with `This operation can only be performed by the Account Holder`, so a new one cannot be minted from here either. **Nothing was relaxed to get past this**: the DMG is deliberately not published, because an un-notarized download is refused by Gatekeeper on the user's own Mac — a broken promise is worse than a late release. The remedy is one of two things only the account holder can do: export the existing `Developer ID Application: Guofeng Liu (U8U443D7ZL)` private key as a `.p12` and import it into this login keychain, or sign in as the Account Holder and let `asc` mint a new one |
