...
| Code signing | Developer ID, notarized; `SMAppService` registration | `codesign --verify --strict` on the app and every nested binary, re-verified inside the DMG by `release.sh` | Developer ID signing **verified** (Guofeng Liu, U8U443D7ZL); notarization submission remains blocked — no `notarytool` keychain profile exists, and storing one needs interactive Apple-ID auth or an API key |
