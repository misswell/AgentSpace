...
public enum DiskUsage {
    ...
    public struct Measurement: Equatable {
        ...
        public init(bytes: UInt64, truncated: Bool, files: Int) {
            self.bytes = bytes
    ...
    public static func allocatedBytes(under path: String, budget: Int = 250_000) -> Measurement {
        ...
        for case let url as URL in enumerator {
            visited += 1
            if visited > budget { return Measurement(bytes: total, truncated: true, files: files) }
        ...
        return Measurement(bytes: total, truncated: false, files: files)
    }
}
